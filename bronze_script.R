...
load("BRONZE_v1.RData")
dim(BRONZE)
# [1] 362  12
head(BRONZE)
#   Cat.No. Chronological_typology  GROUP GROUP2     Cu     Sn    Pb   Zn     Au  Ag   As  Sb
# 1       5                  -1250 ShangA  Shang 729000 176000 56000   88 10.400 600  670 122
# 2      15                  -1200 ShangA  Shang 803000 139000 30800  100  3.050 252  510  77
# 3      16                  -1200 ShangA  Shang 795000 100000 95000  144 18.200 630  710 160
# 4      18                  -1100 ShangA  Shang 765000 106000 84000   47 10.800 550 2400  91
# 5      19                  -1100 ShangA  Shang 752000 164000 49000  500 17.000 550  237  27
# 6      27                  -1250 ShangA  Shang 787000 167000 26000 1670  0.325  81 1020  79
...
group.col <- brewer.pal(3, "Dark2")
group.pch <- c(4,21,22)
group.cex <- c(0.5,0.5,0.5)
group <- BRONZE$GROUP2
group.num <- as.numeric(as.factor(group))
table(group.num)
#   1   2   3 
#  96 109 157 
# Note: the time order in group.num is c(2,3,1)
...
for(igp in 1:3) {
  for(i in group.ends[igp,1]:group.ends[igp,2]) {
    foo <- 100*bronze.pro.cum[time.order[i],]
...
for(igp in 1:3) {
  for(i in group.ends[igp,1]:group.ends[igp,2]) {
    foo <- 100*bronze.sub.pro.cum[time.order[i],]
...
#   factor(group)     PbZn
# 1  Eastern Zhou 7.865428
# 2         Shang 6.602638
# 3  Western Zhou 5.659250
...
#   factor(group)     CuSb
# 1  Eastern Zhou 6.671532
# 2         Shang 8.286858
# 3  Western Zhou 7.558402
...
#   factor(group)     PbSb
# 1  Eastern Zhou 4.899196
# 2         Shang 5.553224
# 3  Western Zhou 4.040664
summary(aov(PbSb ~ factor(group)))
               Df Sum Sq Mean Sq F value Pr(>F)    
factor(group)   2  151.2   75.60   42.38 <2e-16 ***
Residuals     359  640.4    1.78      

### Figure 4, need to re-order dynasties in time order
group.time <- group.num
group.time[group.num==2] <- 1
group.time[group.num==3] <- 2
group.time[group.num==1] <- 3
table(group.time)
group.time
#   1   2   3 
# 109 157  96 
par(mar=c(4.2,4,2,1), mgp=c(2,0.7,0), font.lab=2, mfrow=c(1,3), cex.lab=1.2)
CIplot_uni(PbZn, group=group.time, cols=group.col[c(2,1,3)], names=c("Shang","WZhou","EZhou"), ylab="log(Pb/Zn)",  
           shade=TRUE, ylim=c(5,8.5), thin=1.4, CONF=0.99)
CIplot_uni(CuSb, group=group.time, cols=group.col[c(2,1,3)], names=c("Shang","WZhou","EZhou"), ylab="log(Cu/Sb)", 
           shade=TRUE, ylim=c(6.5,9.0), thin=1.4, CONF=0.99)
CIplot_uni(PbSb, group=group.time, cols=group.col[c(2,1,3)], names=c("Shang","WZhou","EZhou"), ylab="log(Pb/Sb)", 
...
points(bronze.rpc, pch = group.pch[c(2,1,3)][group.time], col = group.col[c(2,1,3)][group.time], font = 1, 
       cex = group.cex[c(2,1,3)][group.time])
text(bronze.csc, labels = colnames(bronze), col = "red", cex = 0.9, font = 4)    
legend("bottomleft", legend=c("S:Shang","WZ:WZou","EZ:EZhou"), bty="n",
       pch=group.pch[c(2,1,3)], col=group.col[c(2,1,3)], pt.cex=group.cex[c(2,1,3)], 
       text.col=group.col[c(2,1,3)], cex=0.9, text.font=2)
for (j in 1:3){
 points     <- bronze.rpc[group.time==j,1:2]
 hpts       <- chull(points)
 hpts       <- c(hpts, hpts[1])
 lines(points[hpts,], lty = 3, col=group.col[c(2,1,3)][j])
}

set.seed(123)
CIplot_biv(bronze.rpc[,1], bronze.rpc[,2], group=group.time, groupcols=group.col[c(2,1,3)], 
           add=TRUE, shade=TRUE, alpha=0.99, 
           groupnames=c("S","WZ","EZ"))
set.seed(123)
CIplot_biv(bronze.rpc[,1], bronze.rpc[,2], group=group.time, groupcols=group.col[c(2,1,3)], 
           alpha=0.99, add=TRUE, shade=FALSE, groupnames=c("S","WZ","EZ"), shownames=FALSE)
...
bronze.BW
#   [1] 0.0000000 0.2362804 0.3680456 0.4422665 0.4924735 0.5326932 0.5615574 0.5870676 0.6121670 0.6310426
...
### Table 2 (columns need re-ordering)
...
# 5                              5 8.190461
# 6                              6 6.945133
ZnSb <- log(bronze[,"Zn"]/bronze[,"Sb"])
aggregate(ZnSb ~factor(bronze.kmeans6$cluster), FUN=mean)
#   factor(bronze.kmeans6$cluster)       ZnSb
# 1                              1  0.1684661
# 2                              2 -1.5694862
# 3                              3 -0.6069485
# 4                              4 -2.3849410
...
points(bronze.ALR.rpc, pch = group.pch[c(2,1,3)][group.time], col = group.col[c(2,1,3)][group.time], font = 1, cex = group.cex[c(2,1,3)][group.time])
text(rescale * bronze.ALR.csc, labels = colnames(bronze.ALR), col = "red", cex = 0.9, font = 4)    
legend("bottomleft", legend=c("S:Shang","WZ:WZou","EZ:EZhou"), bty="n",
       pch=group.pch[c(2,1,3)], col=group.col[c(2,1,3)], pt.cex=group.cex[c(2,1,3)], 
       text.col=group.col[c(2,1,3)], cex=0.9, text.font=2)
for (j in 1:3){
 points     <- bronze.ALR.rpc[group.time==j,1:2]
 hpts       <- chull(points)
 hpts       <- c(hpts, hpts[1])
 lines(points[hpts,], lty = 3, col=group.col[c(2,1,3)][j])
}
require(ellipse)
set.seed(123)
CIplot_biv(bronze.ALR.rpc[,1], bronze.ALR.rpc[,2], group=group.time, groupcols=group.col[c(2,1,3)], 
           add=TRUE, shade=TRUE, alpha=0.99, 
           groupnames=c("S","WZ","EZ"))
set.seed(123)
CIplot_biv(bronze.ALR.rpc[,1], bronze.ALR.rpc[,2], group=group.time, groupcols=group.col[c(2,1,3)], 
...
### STEPR.tmp instead of STEPR --- updat STEPR in easyCODA! #############################################################
chrono <- BRONZE[,2]
chrono.STEPR <- STEPR.tmp(bronze.pro, chrono, method=1)
chrono.STEPR$ratios
      row col
Cu/Sb   1   8
Zn/Au   4   5
...
bronze.groups <- factor(BRONZE$GROUP2, level=c("Shang","Western Zhou","Eastern Zhou"))
...
# 3.469447e-17 0.000000e+00

### perform bootstrapping on the log-contrast coefficients
nboot <- 1000
...
intervalsW
#             Cu         Pb        Sb         Zn         Ag        Au
# 1%  -0.6420077 -0.9988824 0.5752327 -0.2535210 -1.7276157 0.3975181
# 99%  0.1032427 -0.4516482 1.4639894  0.3077239 -0.2940289 1.5536573
intervalsE <- apply(coefs.boot[2,,], 1, quantile, c(0.01,0.99))
colnames(intervalsE) <- c("Cu","Pb","Sb","Zn","Ag","Au")
intervalsE
#             Cu         Pb        Sb         Zn         Ag         Au
# 1%  -2.0824334 -0.2391856 0.5608265 -1.2025526 -0.6049817 0.05993789
# 99% -0.7377898  1.1180257 1.5516270 -0.2212974  0.8539363 1.29579661
...
test <- test[,-1]
train.index <- (1:362)[-test.index]
train <- foo[train.index,]
# remove last column of train, containing the periods
data.x <- train[,-29]
data.y <- chrono[train.index]
...
cvpreds <- rep(0, length(data.y))
for(cv in 1:10) {
  cvset <- (1:nrow(data.x))[-which(groups10==cv)]
  foo <- rpart(data.y[cvset]~., data=data.x[cvset,])
  cvset <- which(groups10==cv)
  foo.pred <- predict(foo, newdata=data.x[cvset,])
...
# applied to test set
foo <- rpart(data.y ~., data = data.x)
foo.pred <- predict(foo, newdata=test[,-29])
...
# remove last column of train, containing the periods
data.x <- train[,-29]
data.y <- bronze.short[train.index]
set.seed(12345)
# we need 242 random allocations
groups10 <- c(rep(1:10, each=24),1:2) 
groups10 <- groups10[sample(1:242)]
cvpreds <- rep(0, length(data.y))
cvpreds <- matrix(0, length(data.y), 3)
cvpreds.test <- matrix(0, length(test.index), 3)
for(cv in 1:10) {
  cvset <- (1:nrow(data.x))[-which(groups10==cv)]
  foo <- rpart(factor(data.y[cvset]) ~ ., data=data.x[cvset,])
  cvset <- which(groups10==cv)
  foo.pred <- predict(foo, newdata=data.x[cvset,])
  cvpreds[cvset,] <- foo.pred
}
cv.pred.period <- rep(0, length(data.y))
for(i in 1:length(data.y)) {
  tmp <- which(cvpreds[i,] == max(cvpreds[i,]))
  if(length(tmp) == 1) cv.pred.period[i] <- tmp
}
table(cv.pred.period, data.y)
#               data.y
# cv.pred.period EZ  S WZ
#              1 46  5 15
#              2  5 50 18
#              3 13 18 72
(46+50+72)/242
...
### 10-fold CV on training data (N=246)
data.x <- as.data.frame(train)
data.y <- bronze.short[-test.index]
...
for(cv in 1:10) {
  ...
  foo <- multinom(data.y[cvset] ~ logratios.Cu.Pb+logratios.Cu.Sb+logratios.Zn.Ag+logratios.Au.Ag, data=data.x[cvset,])
  cvset <- which(groups10==cv)
  foo.pred <- predict(foo, newdata=data.x[cvset,]) 
  cvpreds[cvset] <- foo.pred
}
table(cvpreds, data.y)[c(2,1,3), c(2,1,3)]
#        data.y
# cvpreds  S EZ WZ
#       2 46 10 16
#       1 12 43 13
...
# apply to test data
foo <- multinom(data.y ~ logratios.Cu.Pb+logratios.Cu.Sb+logratios.Zn.Ag+logratios.Au.Ag, data=data.x)
...
test.x <- data.frame(test[,-29])
train.x <- data.frame(train[,-29])
set.seed(123)
bronze.rf <- randomForest(x=train.x,
                       y=factor(bronze.short[train.index]),
                       xtest=test.x,
                       ytest=factor(bronze.short[test.index]),     
...
Confusion matrix:
   EZ  S WZ class.error
EZ 45  2 17   0.2968750
S   6 48 19   0.3424658
WZ 11 12 82   0.2190476
                Test set error rate: 30%
Confusion matrix:
   EZ  S WZ class.error
EZ 28  0  4   0.1250000
S   4 15 17   0.5833333
